...
public class Display extends JComponent implements KeyListener, MouseListener
{
  ...
  private String elementToVisualize;
  
  public Display(final int width, final int height, String element)
  {
    keys = new ConcurrentLinkedQueue<KeyEvent>();
    mouseX = -1;
    mouseY = -1;
    elementToVisualize=element;
    try
    {
      SwingUtilities.invokeAndWait(new Runnable() { public void run() {
        world = new World(width, height, "World", elementToVisualize);
